...
<script>
import EnhancedMaternalPredictionForm from '../components/EnhancedMaternalPredictionForm.vue';
...
export default {
  ...
  methods: {
    ...
    handlePredictionSuccess(predictionData) {
      ...
      // 直接使用从表单组件传递过来的已转换数据，确保正确的数据结构
      const transformedData = {
        comprehensive: {
          overall_risk_level: predictionData.comprehensive.overall_risk_level,
          overall_risk_score: predictionData.comprehensive.overall_risk_score * 100, // 转换为百分比
          risk_description: `您的综合风险评估为${predictionData.comprehensive.overall_risk_level}`,
          recommendations: predictionData.comprehensive.recommendations || [],
          top_risk_factors: predictionData.comprehensive.top_risk_factors || []
        },
        preeclampsia: {
          risk_level: predictionData.preeclampsia?.risk_level || '未知',
          risk_score: predictionData.preeclampsia?.risk_score || 0,
          description: predictionData.preeclampsia?.description || '暂无描述',
          key_factors: predictionData.preeclampsia?.key_factors || []
        },
        gestational_diabetes: {
          risk_level: predictionData.gestational_diabetes?.risk_level || '未知',
          risk_score: predictionData.gestational_diabetes?.risk_score || 0,
          description: predictionData.gestational_diabetes?.description || '暂无描述',
          key_factors: predictionData.gestational_diabetes?.key_factors || []
        },
        preterm_birth: {
          risk_level: predictionData.preterm_birth?.risk_level || '未知',
          risk_score: predictionData.preterm_birth?.risk_score || 0,
          description: predictionData.preterm_birth?.description || '暂无描述',
          key_factors: predictionData.preterm_birth?.key_factors || []
        },
        predictionDate: predictionData.predictionDate // 添加预测日期
      };
      
      this.currentPrediction = transformedData;
      this.showForm = false;
      this.showResult = true;
      this.showNotification('success', '预测成功！点击查看详细结果。');
      
      // 更新统计信息
      this.updateStatistics(predictionData.overall_risk_level);
    },
