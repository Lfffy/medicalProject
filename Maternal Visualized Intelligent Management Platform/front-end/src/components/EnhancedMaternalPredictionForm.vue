  ...
  methods: {
    ...
    validateField(fieldName, value) {
      ...
      for (const rule of rules) {
        // 检查必填
        if (rule.required) {
          // 对于数字类型字段，null、undefined、空字符串都视为未填写
    ...
    async submitForm() {
      ...
      try {
        // 调用预测服务
        console.log('开始调用API...');
        const result = await maternalPredictionService.predictComprehensiveRisk(submitData);
        console.log('API调用返回结果:', result);

        // 转换响应数据以匹配前端展示格式，添加安全的属性检查
        const transformedResult = {
          ...
          gestational_diabetes: result.gestational_diabetes || {},
          preterm_birth: result.preterm_birth || {},
          predictionDate: new Date().toLocaleString()
        };

        // 更健壮的结果处理 - 如果有成功标志或者返回了有效数据，则认为成功
        if (result.success === true || (result.overall_risk_level !== undefined)) {
          // 预测成功
          console.log('预测成功，发送predictionSuccess事件');
          this.$emit('predictionSuccess', transformedResult);
        } else {
          // 预测失败
          console.log('预测失败，返回错误信息');
