...
export default {
  ...
  methods: {
    ...
    checkModelsStatus() {
      maternalPredictionService.getModelStatus()
        .then(response => {
          if (response.success) {
            // 适配不同版本的数据结构
            this.modelsLoaded = response.data.models_loaded || response.data.all_models_loaded;
          }
        })
        .catch(error => {
            ...
            .then(response => {
              ...
              if (response.success || response.code === 200) {
                ...
                this.predictionResults = {
                  comprehensive: responseData.comprehensive || {
                    overall_risk_level: '中风险',
                    overall_risk_score: 0.5,
                    main_risk_factors: ['数据不完整'],
                    suggestions: ['请重新输入完整信息']
                  },
                  preeclampsia: responseData.preeclampsia || {
                    risk_level: '中风险',
                    risk_probability: 0.5,
                    main_risk_factors: ['数据不完整'],
                    suggestions: ['请重新输入完整信息']
                  },
                  gdm: responseData.gdm || {
                    risk_level: '中风险',
                    risk_probability: 0.5,
                    main_risk_factors: ['数据不完整'],
                    suggestions: ['请重新输入完整信息']
                  },
                  preterm: responseData.preterm || {
                    risk_level: '中风险',
                    risk_probability: 0.5,
                    main_risk_factors: ['数据不完整'],
                    suggestions: ['请重新输入完整信息']
                ...
                // 触发预测成功事件
                this.$emit('predictionSuccess', this.predictionResults);
              } else {
                const errorMsg = response.error || response.msg || '预测失败';
                this.$message.error(errorMsg);
                // 触发预测失败事件
                this.$emit('predictionError', { message: errorMsg });
              }
            })
            .catch(error => {
    ...
    resetForm() {
      ...
      this.predictionForm.risk_factors = [];
      this.predictionForm.bmi = null;
    },
    exportResults() {
      try {
        ...
        if (!this.predictionResults) {
          this.$message.warning('没有可导出的预测结果');
          return;
        }
        ...
        const exportFileName = `maternal-risk-prediction-${new Date().toISOString().slice(0, 10)}.json`;
