...
export default {
  ...
  methods: {
    ...
    checkModelsStatus() {
      axios.get('/api/maternal-risk/models/status')
        .then(response => {
          if (response.data.success) {
            this.modelsLoaded = response.data.data.all_models_loaded;
          }
        })
        .catch(error => {
    ...
    predictRisks() {
      this.$refs.predictionForm.validate((valid) => {
        if (valid) {
          ...
          // 预测综合风险
          axios.post('/api/maternal-risk/predict/comprehensive', this.predictionForm)
            .then(response => {
              if (response.data.success) {
                this.predictionResults = response.data.data;
